...
Cellink 引擎提供了一套简单的机制来管理各个节点的流向，保证正确的执行次序。更重要的是，Cellink 引擎能方便地绘制节点之间的关系图（如上所示），让开发者清楚地知道自己所开发的子模块处于整个项目中的什么位置。这种知觉让我在项目实践中对项目产生了极大的把控感。哪怕是面对历史久远的项目，也不会迷失在代码的迷雾中。
...
Cellink 引擎通过搭建“有向无环图”（DAG）对项目流程进行建模。每个任务模块都是有向无环图中的一个节点，节点之间有相互依赖的关系。一个节点都可以直接访问所有父节点的内容（反之不行）。
